...
def search(bunch, pc, task_data, rebel_mode):
	...
	crds, how = task_data
	x, y = crds
	success = achieved_task(assassin[0], assassin[1], x, y, how=='EUROPE', rebel_mode)
...
in_eu = lambda x, y, rebel: (x > 56 or x < 1 or y > 70 or y < 1) if not rebel else (x >= 56 or x <= 1 or y > 70 or y < 1)

def achieved_task(x, y, xt, yt, eu, rebel_mode, r = 1):
	return in_eu(x, y, rebel_mode) if eu else vicinity(x, y, xt, yt, r)
...
def global_act_demote(x):
	...
	x[2][2] = new_type
	return new_type in DEGRADE_UNITS
...
def action(bunch, ass, lvl, pc, rebel_mode):
	intro, colonies, units, nations, tribes, middle, maps, final = bunch
	
	x, y = ass[0], ass[1]
	
	europe_status = in_eu(x, y, rebel_mode)
...
def main():
	...
	rebel_mode = b[0][18] % 2	
	pc = pcs(f)
	result = search(b, pc, load_task(b), rebel_mode)
	if result:
		assassin, lvl = result	
		if lvl > 0:
			action(b, assassin, lvl, pc, rebel_mode)
